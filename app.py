...
PATH_UFS = BASE_PATH + "/municipios_e_uf/BR_UF_2025"
...
# Ajuste a col_demografia para ser uma coluna de informações técnicas:
    ...
    with col_mapa:
        ...
        try:
            # 1. Carregando o Shapefile local do Brasil
            ...
            # --- CAMADA 1: FUNDO (Todos os estados do Brasil em cinza com bordas) ---
            fig_mapa.add_trace(go.Choropleth(
                geojson=geojson_brasil,
                locations=gdf_brasil.index,
                z=[1] * len(gdf_brasil),
                colorscale=[[0, '#F8FAFC'], [1, '#F8FAFC']], # Cinza clarinho
                showscale=False,
            ...
            # --- CAMADA 2: CENTRO-OESTE (Colorido com os dados) ---
            ...
            fig_mapa.add_trace(go.Choropleth(
                ...
                colorbar=dict(
                    ...
                    thickness=15, 
                    len=0.7, # Tamanho relativo para não bugar no layout
                    yanchor="middle", y=0.5, xanchor="left", x=1.02
                )
            ))

            # --- CAMADA 3: NOMES POR EXTENSO (Apenas no CO) ---
            ...
            fig_mapa.update_layout(
                ...
                geo=dict(
                    visible=False,
                    # Forçando a "caixa" de visão usando as coordenadas exatas da região!
            ...
            # --- NOVA ANÁLISE DINÂMICA ---
            if indicador_mapa == "Densidade Demográfica":
                st.markdown("""
                <div style='background-color: #EFF6FF; padding: 15px; border-radius: 8px; border-left: 4px solid #3B82F6; font-size: 13.5px; color: #1E40AF;'>
                    <strong>📊 Análise de Concentração Demográfica:</strong> A densidade no Centro-Oeste é marcada por um contraste agudo. Enquanto o <strong>Distrito Federal</strong> apresenta uma das maiores densidades do país devido à sua natureza urbana e administrativa, o restante da região possui vastos vazios demográficos, típicos de áreas onde o uso do solo é voltado à agropecuária extensiva.
                </div>
                """, unsafe_allow_html=True)
                
            elif indicador_mapa == "PIB per Capita":
                st.markdown("""
                <div style='background-color: #FFF7ED; padding: 15px; border-radius: 8px; border-left: 4px solid #F59E0B; font-size: 13.5px; color: #9A3412;'>
                    <strong>📊 Análise do PIB per Capita:</strong> Observa-se uma clara disparidade: o <strong>Distrito Federal</strong> lidera com rendimentos elevados sustentados pela base de servidores públicos e serviços de alta complexidade. Em contrapartida, estados como Mato Grosso e Goiás, embora apresentem picos de riqueza em seus polos agroindustriais, possuem médias afetadas por uma distribuição de renda mais concentrada nas mãos de grandes produtores.
                </div>
                """, unsafe_allow_html=True)
                
            else: # IDH
                st.markdown("""
                <div style='background-color: #F5F3FF; padding: 15px; border-radius: 8px; border-left: 4px solid #8B5CF6; font-size: 13.5px; color: #5B21B6;'>
                    <strong>📊 Análise do IDH Estadual:</strong> O IDH reflete o sucesso das políticas públicas de estruturação urbana e acesso a serviços. A performance superior do <strong>Distrito Federal</strong> em relação aos demais estados da região destaca a diferença entre uma economia baseada em capital humano institucional e economias que ainda enfrentam desafios para traduzir o crescimento do agronegócio em melhorias sociais generalizadas para a população.
                </div>
                """, unsafe_allow_html=True)
            st.caption("📌 **Nota:** O Centro-Oeste está em destaque. Os contornos ao redor delimitam as rotas logísticas e de escoamento para os estados vizinhos.")
            
        except Exception as e:
            st.error(f"Erro ao carregar o mapa. Verifique o caminho: {PATH_UFS}. Detalhe: {e}")
